...
This parameter testing approach ensures that all exceptions are thrown and all correct behaviour are implemented. Between each unit test, all data in the database are deleted and it is filled with test data, to make the testing environment more realistic, and to decouple tests. If Login is to be tested, it has to create a user first. If a user is already there, we can keep the unit test as simple as possible, only testing for Login. Keeping the unit tests simple and decoupled makes it easier to locate an error. \\ \\
\textbf{System testing}\\*
Every test not contained in the unit test project are done manually. All testing on the GUI are done by reviewing every single use case and trying all possible outcomes. The unit tests ensures that the server functionality works, making it easier to locate errors with manual GUI testing. This is further described in the testing of functional requirements.
